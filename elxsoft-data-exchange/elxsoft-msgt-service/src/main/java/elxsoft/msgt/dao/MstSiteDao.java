...
import elxsoft.com.common.constant.Const;
...
public class MstSiteDao extends BaseJdbcDao
{
	...
	public List<JSONObject> getList(JSONObject jsonParam, int start, int limit)
	{
		...
		sql.append(
				" SELECT MS.*,                                                                                   					");
		sql.append(
				"        (SELECT MC.NAME FROM MST_CODE MC WHERE MC.TYPE ='SITE_TYPE' AND MC.CODE= MS.SITE_TYPE) AS SITE_TYPE_NAME,   ");
		sql.append(
				"        (SELECT MD.DIST_NAME FROM MST_DIST MD WHERE MD.DIST_CODE = MS.DIST_CODE) AS DIST_NAME,  					");
		sql.append(
				"        (SELECT ORG_NAME FROM MST_ORG MO WHERE MO.ORG_ID = MS.ORG_ID) AS ORG_NAME               					");
		sql.append(
				"        FROM MST_SITE MS WHERE 1=1                                                                       			");
		List<Object> sqlArgs = new ArrayList<Object>();
		addWhereCondition(sql, sqlArgs, jsonParam);
		super.appendPageSqlOrcl(sql, start, limit);
	...
	public int insert(JSONObject json) throws Exception
	{
		...
		Object[] args = { super.generateKey(), json.getString("SITE_NAME"), json.getString("SITE_TYPE"),
				json.getString("DIST_CODE"), json.getString("ORG_ID"), json.getString("TEL"), buildDate,
				json.getString("DESC_INFO"), json.getString("MASTER"), json.getString("LINE1"), json.getString("LINE2"),
				json.getString("PARKS"), json.getString("TOTAL_AREA"), json.getString("SITE_IMG"),
				json.getString("DETAIL_BY"), json.getString("DETAIL_BY") };
	...
	public int update(JSONObject json) throws Exception
	{
		...
		Object[] args = { json.getString("SITE_NAME"), json.getString("SITE_TYPE"), json.getString("DIST_CODE"),
				json.getString("ORG_ID"), json.getString("TEL"), buildDate, json.getString("DESC_INFO"),
				json.getString("MASTER"), json.getString("LINE1"), json.getString("LINE2"), json.getString("PARKS"),
				json.getString("TOTAL_AREA"), json.getString("SITE_IMG"), json.getString("DETAIL_BY"),
				json.getString("SITE_ID") };
